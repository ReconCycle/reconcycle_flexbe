...
from robotblockset_python.transformations import rpy2r 
...
class RBS_CMove(EventState):
    ...
    def __init__(self, robot_name, x, t, local_offset=0, global_pos_offset=0, limit_rotations=False):
        super(RBS_CMove, self).__init__(outcomes = ['continue', 'failed'], input_keys = ['t2_data', 'panda_1', 'panda_2'], output_keys = ['t2_out'])
    ...
    def on_enter(self, userdata):
        ...
        self.robot = getattr(userdata,self.robot_name)
        
        if self.x[0] is None:
            #[0]
            position = np.array([userdata.t2_data.position.x,
                                 userdata.t2_data.position.y,
                                 userdata.t2_data.position.z])
            orientation = np.array([userdata.t2_data.orientation.x,
                                    userdata.t2_data.orientation.y,
                                    userdata.t2_data.orientation.z,
                                    userdata.t2_data.orientation.w])
        else : 
            position = self.x[0:3]
            orientation = self.x[3:]
            if len(orientation) == 3 : orientation = rpy2r(orientation, unit = 'deg', out = 'Q')
        ...
        #    self._error = True
        #    return 'failed'       
        x = np.hstack([position, orientation])
        self.robot.CMove(x,  t= self.t, task_space = 'World')
        return 'continue'
