...
class HCA_frame_to_bin(EventState):
    ...
    def __init__(self, init_safe = True, rotate_hca_inside_vise = False):
        super(HCA_frame_to_bin, self).__init__(outcomes = ['continue', 'failed'], input_keys = ['cy', 'dummy'])

        self.init_safe = init_safe
        self.rotate_hca_inside_vise = rotate_hca_inside_vise
        
    def on_enter(self, userdata):

        Logger.loginfo("Entering state HCA_frame_to_bin")
