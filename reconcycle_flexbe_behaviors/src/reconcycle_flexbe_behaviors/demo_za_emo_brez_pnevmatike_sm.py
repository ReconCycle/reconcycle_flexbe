...
'''
class DemozaEMObrezpnevmatikeSM(Behavior):
	'''
	Potrebna oprema:
...
- Ponovi
	...
	def __init__(self):
		super(DemozaEMObrezpnevmatikeSM, self).__init__()
		self.name = 'Demo za EMO brez pnevmatike'
	...
	def create(self):
		...
		_state_machine.userdata.j_slightly_above_slider = "emo/j_slightly_above_slider_3"
		...
		with _sm_joint_move_above_table_3:
			...
			OperatableStateMachine.add('Read object TF',
										...
										transitions={'continue': 'Display Yolact', 'failed': 'failed'},
										autonomy={'continue': Autonomy.Off, 'failed': Autonomy.Off},
		...
		with _sm_cart_move_to_object_4:
			...
			OperatableStateMachine.add('Move pickup pose',
										CallActionTFCartLin(namespace="panda_1", exe_time=2, offset=[-0.03,0.03,-0.06,-102.472,-3.952,-120], offset_type='local', limit_rotations=True),
		...
		_sm_move_up_and_release_hand_5 = ConcurrencyContainer(outcomes=['failed', 'continue'], input_keys=['drop_pose', 'release_pos'], conditions=[
										('continue', [('Release hand all the way', 'continue'), ('move_above', 'continue')]),
										('failed', [('Release hand all the way', 'failed'), ('move_above', 'failed')])
										])

		with _sm_move_up_and_release_hand_5:
			# x:274 y:63
			...
			# x:47 y:47
			OperatableStateMachine.add('Release hand all the way',
		...
		_sm_switch_joint_move_init_6 = OperatableStateMachine(outcomes=['continue', 'failed'], input_keys=['mdb_init_pose'])

		with _sm_switch_joint_move_init_6:
			# x:89 y:44
		...
		# x:30 y:365, x:130 y:365
		_sm_move_init_7 = OperatableStateMachine(outcomes=['continue', 'failed'], input_keys=['release_pos', 'mdb_init_pose'])

		with _sm_move_init_7:
			# x:145 y:61
			OperatableStateMachine.add('switch joint move init',
										_sm_switch_joint_move_init_6,
										transitions={'continue': 'continue', 'failed': 'failed'},
		...
		# x:30 y:365, x:130 y:365
		_sm_move_away_8 = OperatableStateMachine(outcomes=['continue', 'failed'], input_keys=['release_pos', 'mdb_init_pose'])

		with _sm_move_away_8:
			# x:82 y:49
		...
		_sm_place_randomly_9 = OperatableStateMachine(outcomes=['finished', 'failed'], input_keys=['mdb_above_table_pose', 'mdb_init_pose', 'release_pos', 'slight_release_pos'])

		with _sm_place_randomly_9:
			# x:95 y:41
			OperatableStateMachine.add('Move init',
										CallJointTrap(max_vel=self.max_vel, max_acl=self.max_acl, namespace='panda_1'),
			...
			# x:935 y:531
			OperatableStateMachine.add('Move away',
										_sm_move_away_8,
										transitions={'continue': 'finished', 'failed': 'failed'},
										autonomy={'continue': Autonomy.Inherit, 'failed': Autonomy.Inherit},
										remapping={'release_pos': 'release_pos', 'mdb_init_pose': 'mdb_init_pose'})
			...
			# x:959 y:293
			OperatableStateMachine.add('Move up and release hand',
										_sm_move_up_and_release_hand_5,
										transitions={'failed': 'failed', 'continue': 'Move away'},
										autonomy={'failed': Autonomy.Inherit, 'continue': Autonomy.Inherit},
										remapping={'drop_pose': 'drop_pose', 'release_pos': 'release_pos'})

			# x:987 y:185
			OperatableStateMachine.add('open_hand',
										MoveSoftHand(motion_duration=1, motion_timestep=0.05),
										transitions={'continue': 'Move up and release hand', 'failed': 'failed'},
										autonomy={'continue': Autonomy.Off, 'failed': Autonomy.Off},
		...
		_sm_pickup_from_holder_10 = OperatableStateMachine(outcomes=['finished', 'failed'], input_keys=['grab_pos', 'holder_pickup_pose', 'holder_up_pose', 'false', 'true'])

		with _sm_pickup_from_holder_10:
			# x:209 y:74
		...
		_sm_init_holder_and_slider_11 = OperatableStateMachine(outcomes=['failed', 'continue'], input_keys=['false', 'true', 'value'])

		with _sm_init_holder_and_slider_11:
			# x:30 y:51
		...
		# x:30 y:365, x:130 y:365
		_sm_init_controllers_and_move_init_12 = OperatableStateMachine(outcomes=['failed', 'continue'], input_keys=['j_init_pose'], output_keys=['mdb_init_pose'])

		with _sm_init_controllers_and_move_init_12:
			# x:66 y:23
		...
		_sm_concurrent_init_13 = ConcurrencyContainer(outcomes=['failed', 'continue'], input_keys=['false', 'true', 'value', 'j_init_pose', 'open_pos'], output_keys=['mdb_init_pose'], conditions=[
		...
		with _sm_concurrent_init_13:
			# x:77 y:42
			OperatableStateMachine.add('Init holder and slider',
										_sm_init_holder_and_slider_11,
			...
			OperatableStateMachine.add('Init controllers and move init',
										_sm_init_controllers_and_move_init_12,
										transitions={'failed': 'failed', 'continue': 'continue'},
										autonomy={'failed': Autonomy.Inherit, 'continue': Autonomy.Inherit},
		...
		_sm_init_cell_14 = OperatableStateMachine(outcomes=['finished', 'failed'], input_keys=['true', 'open_pos', 'value', 'false', 'j_init_pose'], output_keys=['mdb_init_pose'])

		with _sm_init_cell_14:
			# x:247 y:59
			OperatableStateMachine.add('Concurrent init',
										_sm_concurrent_init_13,
		...
		_sm_close_and_rotate_15 = OperatableStateMachine(outcomes=['finished', 'failed'], input_keys=['true', 'false', 'value'])

		with _sm_close_and_rotate_15:
			# x:47 y:42
			OperatableStateMachine.add('move_slider_back',
										ActivateRaspiDigitalOuput(service_name="/move_slide"),
