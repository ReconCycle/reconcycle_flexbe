...
@author: Boris Kuster, Mihael Simonic
'''
class Demo_za_EMO_Brez_SoftHandSM(Behavior):
	'''
	Demo za EMO brez SoftHand
	'''


	def __init__(self):
		super(Demo_za_EMO_Brez_SoftHandSM, self).__init__()
		self.name = 'Demo_za_EMO_Brez_SoftHand'
	...
	def create(self):
		...
		_sm_open_hand_and_move_init_6 = ConcurrencyContainer(outcomes=['continue', 'failed'], input_keys=['release_pos', 'mdb_init_pose'], conditions=[
										('continue', [('switch joint move init', 'continue')]),
										('failed', [('switch joint move init', 'failed')])
										])
		...
		# x:30 y:365, x:273 y:216, x:230 y:365, x:330 y:365
		_sm_concurrent_init_11 = ConcurrencyContainer(outcomes=['failed', 'continue'], input_keys=['false', 'true', 'value', 'j_init_pose', 'open_pos'], output_keys=['mdb_init_pose'], conditions=[
										('continue', [('Init holder and slider', 'continue'), ('Init controllers and move init', 'continue')]),
										('failed', [('Init controllers and move init', 'failed'), ('Init holder and slider', 'failed')])
										])
